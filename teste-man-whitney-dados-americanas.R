...
####################### Carregando dados #################################
dados <- read.csv("~/04 - Projetos/02 - Python/04 - Case americanas/Teste/joint_base.csv",
                  sep = ';', dec = ',')
...
hist(dados$Custo_Mkt[dados$Departamento == "Bolas de Gude"],
     ylab="Frequência", xlab="Custo MKT", main="Bolas de Gude", probability = T)
lines(density(dados$Custo_Mkt[dados$Departamento == "Bolas de Gude"]))
med = median(dados$Custo_Mkt[dados$Departamento == "Bolas de Gude"])
abline(v = med, col = 'blue')

hist(dados$Custo_Mkt[dados$Departamento == "Cadeiras"],
     ylab="Frequência", xlab="Custo MKT", main="Cadeiras", probability = T)
lines(density(dados$Custo_Mkt[dados$Departamento == "Cadeiras"]))
med = median(dados$Custo_Mkt[dados$Departamento == "Cadeiras"])
abline(v = med, col = 'blue')
...
# IQR = amplitude interquartil. Quanto um quantil varia pro outro
